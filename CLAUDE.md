...
**الإصدار:** Worker `v2.3.1` · الواجهة `v3.0.0`
...
آخر مطابقة: 24-09-2026 · `index.js` v2.3.1 · `index.html` v3.0.0
...
- **`check-log-values.mjs` اتصلّح** — النسخة القديمة كانت بتدوّر على `type:`
  بنقطتين بس، فـ object shorthand مكانش بيتمسك. النسخة الجديدة بتمسك الشكل
  الصريح · shorthand · المفتاح المحسوب · وحالة "نداء أنكور من غير أي مفتاح
  type" (تقرير مش سكوت).
- **قيمة `type='update'` اتسجّلت في `log-values.json`** — كانت مستخدمة فعليًا
  (`handleLog`، نقطة دخول Shopify Flow) بس مش مسجّلة. 🔴 **ملحوظة عن التشيك
  نفسه:** `handleLog` بيكتب بـ `INSERT` مباشر (`env.DB.prepare().bind(...)`
  بقيمة `'update'` موضعية) مش عن طريق `writeLog()` بـ object literal، فالتحقق
  الساكن **مايشوفهوش** — مافيش سطر `type:` هيدوّر عليه. اتأكدت القيمة يدويًا
  بقراية الكود (`grep -n "type"` على `index.js`) وسُجّلت. أي `type` جديد
  يتضاف لنفس المسار (`handleLog` أو أي `INSERT` مباشر تاني) لازم يتسجّل
  **يدويًا** — التحقق الآلي مش هيمسكه.
- **الحارس الديناميكي (الطبقة ٥ — `worker-builder` Step 7-ج) اتضاف** —
  `LOG_REGISTRY` (`§LOG-REG` فوق `§SHARED`) + `noteUnregisteredLogValues` +
  UPSERT صامت في `log_value_alerts`. اتحط في **موضعين**: `writeLog()`
  (بتاعة `login`/`logout`) **و** `handleLog` مباشرة (بتاعة `update`، لأنها
  مش بتعدّي على `writeLog()`). مفيش رفض كتابة أبدًا — قيمة غير مسجّلة بتتعلّم
  بـ `extra._unregistered` وتتكتب عادي.
